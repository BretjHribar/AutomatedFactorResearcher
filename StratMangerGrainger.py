...
def evalForGraphReturns(individual):
    ...
    #g_input = pd.DataFrame([target.iloc[:, 0].fillna(0), out_normalzed.iloc[:, 0].fillna(0)]).transpose()
    #testStart
    g_input = pd.DataFrame([target.iloc[testStart:, :].shift(1).fillna(0).stack().values,
                            out_normalzed.iloc[testStart:, :].fillna(0).stack().values]).transpose()
    GC_test = grangercausalitytests(g_input,1, verbose := True)
    grainger_causality_p_value = GC_test[1][0]['ssr_ftest'][1]
    print(f"grainger_causality_p_value: {grainger_causality_p_value}")

    return ( DFreturns, out_normalzed, DFreturnsRowSum, grainger_causality_p_value)
########################################################################
########################################################################
def ReturnY():
    open = df_open
...
def main():
    ...
    for row in alphas:
        print(row)
        [A, B, C, G] = evalForGraphReturns(row['alphastring'])
        if G < 0.01:
            NANs = B.isna().sum().sum()
            print("NANs: ", NANs)
