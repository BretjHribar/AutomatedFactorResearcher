"""
live_paper_trade.py — Live paper trading using the Adaptive Net Factor Returns
strategy on 5-minute Polymarket crypto candle contracts.

Architecture:
  1. Binance WebSocket  → streaming 5m klines for signal computation
  2. Polymarket CLOB API → real orderbooks for execution pricing
  3. Alpha primitives   → identical to iterate_5m_v2.py (ts_zscore, delta, etc.)
  4. Adaptive Net       → rolling net factor returns weighting (Isichenko-style)

The strategy:
  - Maintains a rolling buffer of 5m bars per asset
  - On each bar close, computes all alpha signals (shifted by 1 → no lookahead)
  - Weights each alpha by its rolling net-of-fee expected return
  - Only alphas with positive net ER get weight
  - Combined signal direction determines UP/DOWN bet
  - Trades on Polymarket CLOB at real bid/ask prices

Usage:
    python live_paper_trade.py                 # Run with all 3 assets
    python live_paper_trade.py --symbol ETH    # ETH only
    python live_paper_trade.py --stats         # Show summary
"""
...
PAPER_TRADE_LOG = Path(__file__).parent / "paper_trades_5m.jsonl"
PAPER_STATE_FILE = Path(__file__).parent / "paper_state_5m.json"

# Adaptive Net strategy params (from iterate_5m_v2.py optimization)
CONFIGS = {
    "BTCUSDT": {"corr_cutoff": 0.80, "max_alphas": 15, "lookback": 5760, "phl": 36},
    "ETHUSDT": {"corr_cutoff": 0.80, "max_alphas": 10, "lookback": 5760, "phl": 12},
    "SOLUSDT": {"corr_cutoff": 0.70, "max_alphas": 15, "lookback": 2880, "phl": 12},
}
...
class PaperTrader:
    ...
    def open_position(self, symbol, candle_open_time, direction,
        ...
        pm_data = {}
        entry_price = 0.50
        fee = BLENDED_TAKER_FEE * BASE_TRADE_SIZE
    ...
    def close_position(self, trade_id, outcome):
        ...
        if direction == "UP":
            if outcome == 1:
                pnl = (1.0 - entry_price) * BASE_TRADE_SIZE - fee
                trade["result"] = "WIN"
            else:
                pnl = -entry_price * BASE_TRADE_SIZE - fee
                trade["result"] = "LOSS"
        else:
            if outcome == 0:
                pnl = entry_price * BASE_TRADE_SIZE - fee
...
async def run_live_feed(symbols_filter=None):
    ...
    print(f"\n{'='*72}")
    print(f"  POLYMARKET 5m CANDLE — ADAPTIVE NET FACTOR RETURNS")
    print(f"  Live Paper Trading with Real CLOB Integration")
    print(f"{'='*72}")
    print(f"  Symbols:  {', '.join(SYMBOL_NAMES[s] for s in active_symbols)}")
    print(f"  Interval: {INTERVAL}")
    print(f"  Strategy: Adaptive Net Factor Returns (Isichenko-style)")
    print()
