"""
univariate_hf_v8_mtf.py — Multi-Timeframe Adaptive Net (V8)

PRODUCTION STRATEGY: 1H+4H signals, Isichenko-style adaptive weighting.

Architecture:
  - 198+ alpha signals across 10 categories on 1H
  - ~40 additional signals from 4H (safe alignment: shifted by 1 period)
  ...
  - Walk-forward: 8-month rolling train, 1-month test

Walk-Forward OOS Results (Jun 2024 - Mar 2025):
  BTC: +5.31  ETH: +6.06  SOL: +6.05  BNB: +4.77  DOGE: +7.06
  COLLECTIVE: +9.93
...
DATA_DIR = Path(__file__).parent / 'data' / 'binance_cache' / 'klines'
...
# ===========================================================================
# MULTI-TIMEFRAME ALPHA GENERATION
# ===========================================================================

def build_mtf_alphas_safe(df_1h, df_4h):
    """
    Multi-timeframe alpha matrix: 1H signals + SAFE 4H signals.
    
    CRITICAL: 4H data is SHIFTED by 1 period (4 hours) before reindexing
    to 1H grid, preventing lookahead bias. All 1H signals are also shifted
    by 1 bar as standard.
    """
    ...
    uwick = high - pd.concat([close, opn], axis=1).max(axis=1)
    lwick = pd.concat([close, opn], axis=1).min(axis=1) - low
    ...
    # === 4H SIGNALS (SAFE: shifted by 1 period to prevent lookahead) ===
    c4 = df_4h['close'].shift(1)  # CRITICAL: only use CLOSED 4H bars
    h4 = df_4h['high'].shift(1)
    l4 = df_4h['low'].shift(1)
    lr4 = np.log(c4 / c4.shift(1))
    cp4 = safe_div(c4 - l4, h4 - l4)
    ...
    for mw in [3,6,12]:
        for dc in [0.9,0.95,0.98]:
            sig = decay_exp((c4 - c4.shift(mw)) * cp4, dc).reindex(
                df_1h.index, method='ffill')
            alphas[f'h4_dec{dc}_d{mw}_cp'] = sig
    
    alpha_df = pd.DataFrame(alphas, index=df_1h.index)
    alpha_df = alpha_df.shift(1)  # Standard 1-bar lookahead prevention
    return alpha_df
...
def walk_forward(sym, train_months=8, verbose=True):
    """Run walk-forward backtest with multi-timeframe signals."""
    df15 = pd.read_parquet(DATA_DIR / '15m' / f'{sym}.parquet')
    ...
    for i in range(len(months) - 1):
        ...
        if len(train_1h) < 2000 or len(test_1h) < 400:
            continue
        ...
        # Phase 2: Parameter selection on TRAIN
        best_sr = -999
        best_cfg = None
        best_sel = None
                ...
                for lb in [180, 240, 360, 480, 720]:
                    for phl in [1, 3, 6, 12]:
                        mt = strategy_adaptive_net(
                            alpha_tr, train_ret, sel,
                            lookback=lb, phl=phl)
                        if mt and mt['sharpe'] > best_sr:
                            best_sr = mt['sharpe']
                            best_cfg = {'cc': cc, 'mn': mn, 'lb': lb, 'phl': phl}
                            best_sel = sel
        ...
        mte = strategy_adaptive_net(
            alpha_te, test_ret, best_sel,
            lookback=best_cfg['lb'], phl=best_cfg['phl'])
...
if __name__ == '__main__':
    print('#' * 70)
    print('# V8: MULTI-TIMEFRAME ADAPTIVE NET (1H + 4H)')
    print('# Walk-forward: 8-month train, 1-month test')
    ...
    for sym in SYMBOLS:
        t0 = time.time()
        print(f'\n{sym}:')
        pnl, details, sh = walk_forward(sym)
        if pnl is not None:
            all_pnl[sym] = pnl
    ...
    if all_pnl:
        ...
        port_df.to_parquet(Path(__file__).parent / 'data' / 'v8_mtf_pnl.parquet')
        print('Saved to data/v8_mtf_pnl.parquet')
