"""
TOP2000 PIT, D=34 — original 24 + 10 truly INDEPENDENT chars (not redundant
variants of vol/momentum/value already in the set). Gamma rescaled by sqrt(24/D).
"""
...
# Original 24 + 10 truly independent chars (intraday range, volume dynamics,
# beta, debt-to-assets — none redundant with existing chars)
D34 = [
    # Original 24
    "log_returns",
    ...
    # 10 INDEPENDENT additions ----------------------------------------------
    # Intraday range/microstructure (4)
...
base.CHAR_NAMES = D34
GAMMA_REF_D = 24
GAMMA_SCALE = float(np.sqrt(GAMMA_REF_D / len(D34)))
print(f"D={len(D34)}  GAMMA_SCALE = sqrt({GAMMA_REF_D}/{len(D34)}) = {GAMMA_SCALE:.4f}", flush=True)
...
LOG = base.RESULTS_DIR / "voc_equities_pit_d34.csv"


def main():
    t0 = time.time()
    print(f"TOP2000 PIT D=34 (24 original + 10 INDEPENDENT) + gamma-rescaled, P sweep {P_GRID}", flush=True)
