"""
V9: 4-timeframe strategy (1H + 2H + 4H + 8H) + cross-asset signals + ensemble top-K
"""
...
def build_1h_alphas(df_1h):
    """Core 1H alpha library (~200 signals)."""
    ...
    for w in [5,10,20]: alphas[f'atr_mr_{w}']=-safe_div(delta(close,w),atr)
    # Regime
    ...
    # NEW: Acceleration signals
    for w in [3,5,8,12]:
        mom_w = ts_sum(log_ret, w)
        alphas[f'accel_{w}'] = mom_w - mom_w.shift(w)  # momentum of momentum
    # NEW: Volume acceleration
    vol_ma5 = volume.rolling(5).mean()
    vol_ma20 = volume.rolling(20).mean()
    alphas['vol_accel'] = safe_div(vol_ma5, vol_ma20) - 1
    # NEW: Intrabar volatility ratio
    ib_vol = safe_div(high - low, close.shift(1))
    for w in [10, 20]:
        alphas[f'ibvol_z_{w}'] = -ts_zscore(ib_vol, w)  # low intrabar vol = mean revert
    # NEW: Consecutive direction
    direction = np.sign(ret)
    for w in [3, 5, 8]:
        alphas[f'consec_{w}'] = -direction.rolling(w).sum() / w  # contrarian on streaks
    return alphas

def build_cross_asset_signals(all_1h, sym, df_1h):
    """Cross-asset signals: BTC as market factor."""
    if sym == 'BTCUSDT' or 'BTCUSDT' not in all_1h:
        return {}
    btc = all_1h['BTCUSDT']
    common = df_1h.index.intersection(btc.index)
    if len(common) < 500:
        return {}
    btc_ret = btc['close'].pct_change().loc[common]
    sym_ret = df_1h['close'].pct_change().loc[common]
    btc_lr = np.log(btc['close'] / btc['close'].shift(1)).loc[common]
    alphas = {}
    # BTC momentum as signal for alts
    for w in [3,5,8,12,24]:
        alphas[f'btc_mom_{w}'] = ts_sum(btc_lr, w).reindex(df_1h.index, method='ffill')
    # BTC mean reversion for alts
    for w in [5,10,20,30]:
        alphas[f'btc_mr_{w}'] = (-ts_zscore(btc['close'].loc[common], w)).reindex(df_1h.index, method='ffill')
    # Relative strength: how much this coin outperformed BTC
    for w in [5,12,24,48]:
        rel = (ts_sum(sym_ret, w) - ts_sum(btc_ret, w))
        alphas[f'rel_str_{w}'] = (-rel).reindex(df_1h.index, method='ffill')  # mean revert outperformance
    # Beta-adjusted return
    rolling_beta = sym_ret.rolling(72, min_periods=24).cov(btc_ret) / btc_ret.rolling(72, min_periods=24).var()
    residual = sym_ret - rolling_beta * btc_ret
    for w in [5,10,20]:
        alphas[f'resid_mr_{w}'] = (-ts_zscore(residual.cumsum(), w)).reindex(df_1h.index, method='ffill')
    return alphas
...
print('='*70)
print('V9: 4-TIMEFRAME + CROSS-ASSET + ENSEMBLE')
print('='*70)

# Pre-load all 1H data for cross-asset signals
all_1h = {}
all_data = {}
for sym in SYMBOLS:
...
for sym in SYMBOLS:
    ...
    for i in range(len(months)-1):
        ...
        # Build alpha matrix: 1H + 2H + 4H + 8H + cross-asset
        alphas_1h = build_1h_alphas(train_1h)
        train_2h = df_2h.loc[str(train_start):str(test_start)]
        train_4h = df_4h.loc[str(train_start):str(test_start)]
        train_8h = df_8h.loc[str(train_start):str(test_start)]
        alphas_2h = build_htf_signals(train_2h, train_1h, 'h2', shift_n=1)
        alphas_4h = build_htf_signals(train_4h, train_1h, 'h4', shift_n=1)
        alphas_8h = build_htf_signals(train_8h, train_1h, 'h8', shift_n=1)
        alphas_cross = build_cross_asset_signals(all_1h, sym, train_1h)
        
        all_alphas = {**alphas_1h, **alphas_2h, **alphas_4h, **alphas_8h, **alphas_cross}
        alpha_tr = pd.DataFrame(all_alphas, index=train_1h.index).shift(1)
        ...
        # Test
        test_2h = df_2h.loc[str(test_start):str(test_end)]
        test_4h = df_4h.loc[str(test_start):str(test_end)]
        test_8h = df_8h.loc[str(test_start):str(test_end)]
        alphas_1h_te = build_1h_alphas(test_1h)
        alphas_2h_te = build_htf_signals(test_2h, test_1h, 'h2', shift_n=1)
        alphas_4h_te = build_htf_signals(test_4h, test_1h, 'h4', shift_n=1)
        alphas_8h_te = build_htf_signals(test_8h, test_1h, 'h8', shift_n=1)
        alphas_cross_te = build_cross_asset_signals(all_1h, sym, test_1h)
        all_alphas_te = {**alphas_1h_te, **alphas_2h_te, **alphas_4h_te, **alphas_8h_te, **alphas_cross_te}
        alpha_te = pd.DataFrame(all_alphas_te, index=test_1h.index).shift(1)

        mte=strategy_adaptive_net(alpha_te,test_ret,best_sel,lookback=best_cfg['lb'],phl=best_cfg['phl'])
        if mte:
