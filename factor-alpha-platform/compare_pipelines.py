...
def run_equal_weight(features, alpha_signals, fee_bps=5.0):
    ...
    for t in range(start_bar, n_bars):
        ...
        forecast = forecast.replace([np.inf, -np.inf], 0).fillna(0)
        forecast = forecast.sub(forecast.mean())
        
        # Optimize
        ret_slice = returns[universe].iloc[max(0, t-120):t]
        ret_slice = ret_slice.replace([np.inf, -np.inf], 0).fillna(0)
        
        max_to = 2.0 if fee_bps == 0.0 else 0.08
        holdings, converged = optimize_portfolio(
            forecast, ret_slice, current_holdings=prev_holdings,
            tx_cost_bps=fee_bps, max_turnover=max_to
        )
...
def run_walkforward_qp(features, alpha_signals, fee_bps=5.0,
    ...
    for t in range(start_bar, n_bars):
        ...
        # Relax turnover if zero fees so the optimizer isn't constrained
        max_to = 2.0 if fee_bps == 0.0 else 0.08
