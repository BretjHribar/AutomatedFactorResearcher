...
def run_strategy(strategy_name, **kwargs):
    ...
    if strategy_name in PROPER_STRATEGIES:
        ...
        if raw_signals is None:
            return None
        n = len(raw_signals)
        print(f"\nRunning '{strategy_name}' on {n} alphas (RAW), validation ({VAL_START} to {VAL_END}), {VAL_FEES}bps fees")
        result, label = PROPER_STRATEGIES[strategy_name](raw_signals, returns_pct, close, universe, **kwargs)
    else:
        signals, returns_pct, close, universe = load_alpha_signals()
        if signals is None:
            return None
        n = len(signals)
        print(f"\nRunning '{strategy_name}' on {n} alphas, validation ({VAL_START} to {VAL_END}), {VAL_FEES}bps fees")
...
def compare_all():
    ...
    print(f"Validation ({VAL_START} to {VAL_END}), {VAL_FEES}bps fees")
    ...
    if raw_signals:
        ...
        for mw in [0.02, 0.03, 0.05, 0.08]:
            for mc in [0.30, 0.40, 0.50, 0.60, 0.70]:
                try:
                    result, label = strategy_corr_select(raw_signals, returns_pct, close, universe,
                                                         max_wt=mw, max_corr=mc)
                    results.append((label, result.sharpe, result.fitness, result.turnover, result.max_drawdown))
                except Exception as e:
                    print(f"  CorrSelect(mw={mw},mc={mc}) failed: {e}")
        ...
        for mw in [0.02, 0.03, 0.05, 0.08]:
            for mc in [0.30, 0.40, 0.50, 0.60]:
                for lb in [180, 240, 280]:
                    try:
                        result, label = strategy_corr_select_adaptive(raw_signals, returns_pct, close, universe,
                                                                       max_wt=mw, max_corr=mc, lookback=lb)
                        results.append((label, result.sharpe, result.fitness, result.turnover, result.max_drawdown))
                    except Exception as e:
                        print(f"  CorrSelAdaptive(mw={mw},mc={mc},lb={lb}) failed: {e}")
...
def main():
    ...
    run_strategy(args.strategy, lookback=args.lookback, top_n=args.top, decay=args.decay)
