"""
V8 Re-run using FRESH Binance Futures 15m data (downloaded 2026-03-11).
Data source: fapi.binance.com/fapi/v1/klines
"""
import warnings; warnings.filterwarnings('ignore')
import numpy as np, pandas as pd, time

DATA_DIR = 'data/binance_futures_15m'  # FRESH download from fapi.binance.com
...
def build_mtf_alphas_safe(df_1h, df_4h):
    ...
    # 4H signals (SAFE: shifted by 1 period)
...
print('='*70)
print('V8 RE-RUN: FRESH BINANCE FUTURES 15m DATA')
print(f'Source: fapi.binance.com (downloaded 2026-03-11)')
print(f'Fee: {FEE_BPS} bps on position changes')
print('='*70)
...
if len(all_pnl)>0:
    ...
    print(f'\nCOLLECTIVE (V8 fresh futures): {coll_sh:+.2f}')
    for sym in SYMBOLS:
        if sym in all_pnl:
            d=all_pnl[sym].resample('1D').sum();d=d[d!=0]
            sh=d.mean()/d.std()*np.sqrt(365) if len(d)>10 else 0
            print(f'  {sym}: {sh:+.2f}')
    port_df.to_parquet('data/v8_fresh_futures_pnl.parquet')
    print('Saved to data/v8_fresh_futures_pnl.parquet')
