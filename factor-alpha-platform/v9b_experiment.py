"""
V9b: V9 improvements with focus on maximizing collective Sharpe.
Changes from V9:
  1. Add 12H timeframe
  2. More cross-asset signals (ETH, SOL as secondary factors for all)
  3. Per-symbol train window optimization (try 6, 8, 10 months)
  4. Longer lookback range (up to 1440)
  5. More aggressive corr cutoff exploration (0.30-0.95)
"""
...
def build_cross_asset_all(all_1h, sym, df_1h):
    """Cross-asset signals using BTC + ETH + SOL as factors."""
    alphas = {}
    for factor_sym in ['BTCUSDT', 'ETHUSDT', 'SOLUSDT']:
        ...
        for w in [3,5,8,12,24]:
            alphas[f'{pfx}_mom_{w}'] = ts_sum(fac_lr, w).reindex(df_1h.index, method='ffill')
        for w in [5,10,20]:
            alphas[f'{pfx}_mr_{w}'] = (-ts_zscore(fac['close'].loc[common], w)).reindex(df_1h.index, method='ffill')
        for w in [5,12,24]:
...
print('='*70)
print('V9b: 5-TIMEFRAME + FULL CROSS-ASSET + WIDER SWEEP')
print('='*70)
...
for sym in SYMBOLS:
    ...
    df_12h=df15.resample('12h').agg({'open':'first','high':'max','low':'min','close':'last','volume':'sum','quote_volume':'sum','taker_buy_volume':'sum','taker_buy_quote_volume':'sum'}).dropna()
    ...
    for i in range(len(months)-1):
        ...
        # Try multiple train windows, pick best
        best_sr_global=-999;best_cfg_global=None;best_sel_global=None;best_tw=None
        
        for train_months in [6, 8, 10]:
            train_start=test_start-pd.DateOffset(months=train_months)
            train_1h=df_1h.loc[str(train_start):str(test_start)]
            if len(train_1h)<1500: continue
            ...
            tr_12h = df_12h.loc[str(train_start):str(test_start)]
                    ...
                    for lb in [120,180,240,360,480,720,1440]:
                        for phl in [1,2,3,6]:
                            ...
                            if mt and mt['sharpe']>best_sr_global:
                                ...
                                best_cfg_global={'cc':cc,'mn':mn,'lb':lb,'phl':phl,'tw':train_months}
                                best_sel_global=sel
                                best_tw=train_months

        if best_cfg_global is None: continue
        
        # Test with best train window
        test_1h=df_1h.loc[str(test_start):str(test_end)]
        if len(test_1h)<400: continue
        ...
        mte=strategy_adaptive_net(alpha_te,test_ret,best_sel_global,lookback=best_cfg_global['lb'],phl=best_cfg_global['phl'])
        if mte:
            fold_pnls.append(mte['pnl_series'])
            fold_details.append({'fold':test_start.strftime('%Y-%m'),**mte,'tw':best_tw})
    ...
    if fold_pnls:
        ...
        for f in fold_details:
            tw = f.get('tw','?')
            print(f'  {f["fold"]}: SR={f["sharpe"]:+.1f} WR={f["win_rate"]:.1%} PnL={f["net_pnl_bps"]:+.0f}bps tw={tw}m')
