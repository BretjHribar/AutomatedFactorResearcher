"""
TOP2000 PIT, D=30 — chars selected methodically on TRAIN ONLY by univariate
IC + redundancy filter (corr < 0.7). γ rescaled by sqrt(24/30). P sweep.
"""
...
selection = json.load(open(base.RESULTS_DIR / "selected_chars_train.json"))
SELECTED = selection["selected"]
SIGNS = selection["signs"]
print(f"Selected D = {len(SELECTED)}", flush=True)
...
LOG = base.RESULTS_DIR / "voc_equities_pit_d30_selected.csv"
...
def patched_load_data():
    ...
    # Apply signs to chars (so negative-IC chars enter as -z)
    for c in SELECTED:
    ...
    print(f"  D={len(available)}/{len(SELECTED)}  missing={missing}", flush=True)
...
def main():
    t0 = time.time()
    print(f"TOP2000 PIT D=30 (TRAIN-selected) + γ-rescaled, P sweep {P_GRID}", flush=True)
