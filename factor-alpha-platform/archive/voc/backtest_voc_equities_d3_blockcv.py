"""TOP2000 PIT D=3 — chars selected by block-CV forward greedy on TRAIN."""
...
P_GRID = [200, 500, 1000, 2000]
LOG = base.RESULTS_DIR / "voc_equities_pit_d3_blockcv.csv"
...
def patched_load_data():
    ...
    print(f"  D={len(available)}/{len(SELECTED)}  T={len(dates)} N={len(tickers)}", flush=True)
...
def main():
    t0 = time.time()
    print(f"TOP2000 PIT D={len(SELECTED)} (block-CV) + γ-rescaled, P sweep {P_GRID}", flush=True)
