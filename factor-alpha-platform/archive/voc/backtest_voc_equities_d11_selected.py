"""
TOP2000 PIT, D=11 — chars selected on TRAIN by FORWARD GREEDY combined IC.
γ rescaled by sqrt(24/11). P sweep.
"""
...
selection = json.load(open(base.RESULTS_DIR / "selected_chars_combined_ic.json"))
...
P_GRID = [500, 1000, 2000, 5000]   # smaller D so test smaller P first
LOG = base.RESULTS_DIR / "voc_equities_pit_d11_selected.csv"
...
def main():
    t0 = time.time()
    print(f"TOP2000 PIT D=11 (forward-greedy combined-IC) + γ-rescaled, P sweep {P_GRID}", flush=True)
