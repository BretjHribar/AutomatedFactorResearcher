"""
AIPT (DKKM) Random Fourier Features + ridge-Markowitz on Binance daily.

Universe: BINANCE_TOP100 (point-in-time, daily rebalance)
Frequency: 1d
History: 2019-09 → 2026-04 (~2400 bars)

Mirrors backtest_voc_postfix.py mode-1 (chars only) but:
  - INTERVAL = 1d
  - daily rebal
  - sqrt(365) annualization (crypto trades 7d/wk)
"""
...
INTERVAL        = "1d"
BARS_PER_YEAR   = 365.0          # 7-day crypto market
TRAIN_BARS      = 750            # ~2 years
MIN_TRAIN_BARS  = 250            # ~8 months minimum
REBAL_EVERY     = 1              # daily
OOS_START       = "2022-09-01"   # ~3 yr after BTCUSDT launch
COVERAGE_CUTOFF = 0.30
Z_RIDGE         = 1e-3
SEED            = 42
GAMMA_GRID      = [0.5, 0.6, 0.7, 0.8, 0.9, 1.0]
TAKER_BPS       = 3.0            # 3 bps per side
P               = 1000
...
MATRICES_DIR  = PROJECT_ROOT / "data/binance_cache/matrices"
UNIVERSE_PATH = PROJECT_ROOT / "data/binance_cache/universes" / f"{UNIVERSE}.parquet"
RESULTS_DIR   = PROJECT_ROOT / "data/aipt_results/binance_daily"
...
def load_data():
    ...
    print(f"  Date range: {dates.min().date()} -> {dates.max().date()}", flush=True)
...
def main():
    t0 = time.time()
    print("=" * 80)
    print("BINANCE DAILY AIPT  (P=1000, chars-only)")
    print("=" * 80)
    ...
    print(f"  oos_start_idx={oos_start_idx} ({dates[oos_start_idx].date()}), "
          f"start_bar={start_bar} ({dates[start_bar].date()})", flush=True)
    ...
    if df.empty:
        print("  EMPTY result — check config")
        return

    df["date"] = [dates[i] for i in df["bar_idx"]]
    df.to_csv(RESULTS_DIR / "binance_daily_returns.csv", index=False)
    m = metrics(df)
    print("\n## METRICS")
    for k, v in m.items():
        print(f"  {k:<24} {v if isinstance(v,int) else f'{v:.4f}'}")
    json.dump(m, open(RESULTS_DIR / "binance_daily_metrics.json", "w"), indent=2)
    ...
    ax.set_yscale("log")
    ax.set_title(f"Binance Daily AIPT — P={P}, D={D}, {dates[oos_start_idx].date()} → {df['date'].iloc[-1].date()}\n"
                 f"Gross SR={m['gross_sharpe']:.2f}  Net3bps SR={m['net_3bps_sharpe']:.2f}  TO={m['avg_turnover']*100:.1f}%/d")
    ax.legend()
    ax.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig(RESULTS_DIR / "binance_daily_equity.png", dpi=110)
    plt.close(fig)
