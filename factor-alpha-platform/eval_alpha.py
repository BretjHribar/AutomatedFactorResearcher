...
NEUTRALIZE   = "market"
BARS_PER_DAY = 6
COVERAGE_CUTOFF = 0.0   # was 0.3 — disabled because kucoin universe is now history-filtered constant top-N by ADV; the cov filter would re-introduce late-listing exclusion bias

# Quality gates
MIN_IS_SHARPE  = 2.5  # was 3.0 — lowered after universe re-construction (proper TOP30 ceiling is ~+2.5 SR per H1/R1 sweeps)
MIN_FITNESS    = 5.0
...
def main():
    ...
    if args.universe:
        ...
        if args.universe.startswith("KUCOIN"):
            ...
            EXCHANGE = "binance"
            # Binance has data from 2019-09 — use a longer TRAIN window covering
            # the 2021 bull peak, 2022 LUNA/FTX bear, 2023 recovery, 2024-25 bull.
            # H1/H2 split places one bear-and-recovery in each half so an alpha
            # must work across genuinely distinct regimes (not just one bull leg).
            TRAIN_START = "2021-01-01"
            TRAIN_END   = "2025-09-01"
            SUBPERIODS  = [
                ("2021-01-01", "2023-05-01", "H1"),
                ("2023-05-01", "2025-09-01", "H2"),
            ]
        _DATA_CACHE.clear()
