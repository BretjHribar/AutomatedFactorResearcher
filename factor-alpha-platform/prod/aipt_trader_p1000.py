"""
prod/aipt_trader_p1000.py -- AIPT SDF Paper Trader, P=1000 instance.

Identical to aipt_trader.py except:
  - P_FACTORS = 1000
  - STATE_DIR  → prod/state/aipt_p1000
  - LOG_DIR    → prod/logs/kucoin/aipt_p1000

Run on the same 4h schedule as aipt_trader.py (independent state).
"""
...
P_FACTORS       = 1000      # ← only change vs aipt_trader.py
Z_RIDGE         = 1e-3
SEED            = 42
...
# Separate dirs from the P=100 instance
STATE_DIR     = PROJECT_ROOT / "prod" / "state" / "aipt_p1000"
LOG_DIR       = PROJECT_ROOT / "prod" / "logs" / "kucoin" / "aipt_p1000"
...
def setup_logging():
    log_file = LOG_DIR / f"aipt_p1000_{dt.datetime.utcnow().strftime('%Y%m%d_%H%M')}.log"
    logging.basicConfig(
        level=logging.INFO,
    ...
    return logging.getLogger("aipt_trader_p1000")

log = setup_logging()

# ============================================================================
# RFF / SDF CORE (identical math to aipt_trader.py)
# ============================================================================
...
def build_characteristics_vector(matrices: dict, tickers: list, bar_idx: int):
...
def compute_rff_signals(Z: np.ndarray, theta: np.ndarray, gamma: np.ndarray, P: int):
    proj = Z @ theta.T
...
def estimate_ridge_markowitz(F_train: np.ndarray, z: float):
    T, P = F_train.shape
...
def save_state(lambda_hat, prev_weights, bars_since_rebal, last_bar_time, tickers):
    state = {
...
def load_state():
    ...
    if prev_weights.size == 0:
        prev_weights = None
    return {**state, "lambda_hat": lambda_hat, "prev_weights": prev_weights}
...
def log_trade(timestamp, bar_time, target_weights, prev_weights, turnover,
    ...
    equity_file = EQUITY_DIR / "equity_aipt_p1000.csv"
    ...
    trade_path = TRADE_LOG_DIR / f"aipt_p1000_{timestamp.replace(':', '-')}.json"
    target_dict = {sym: round(float(w), 6) for sym, w in target_weights.items()
                   if abs(w) > 1e-6}
    trade_log = {
        "timestamp": timestamp,
...
def run_pipeline():
    ...
    log.info("=" * 70)
    log.info(f"  AIPT TRADER P=1000 -- {now.strftime('%Y-%m-%d %H:%M')} UTC")
    ...
    if t_last > 0:
        ...
        valid = (~np.isnan(Z_prev).any(axis=1)
                 & ~np.isnan(close_vals[t_last])
                 & ~np.isnan(close_vals[t_last - 1]))
        N_t = valid.sum()
    ...
    if bars_since_rebal >= REBAL_EVERY or lambda_hat is None:
        train_data = [fr for (ts, fr) in fr_history if ts < last_bar_time]
...
def run_backfill():
    ...
    log.info("=" * 70)
    log.info(f"  AIPT P=1000 BACKFILL -- building factor return history")
    log.info("=" * 70)
    ...
    oos_idx = next((i for i, d in enumerate(dates) if str(d) >= OOS_START), None)
    start_bar = max(0, oos_idx - TRAIN_BARS - 10)

    log.info(f"  N={N}, D={D}, P={P_FACTORS}, T={T}, OOS start={oos_idx}")
    ...
    for t in range(start_bar, T - 1):
        ...
        valid = (~np.isnan(Z_t).any(axis=1)
                 & ~np.isnan(close_vals[t])
                 & ~np.isnan(close_vals[t + 1]))
        N_t = valid.sum()
        if N_t < 5:
            continue
    ...
    if len(train_data) >= MIN_TRAIN_BARS:
        ...
        abs_sum = np.abs(raw_w).sum()
        w_norm = raw_w / abs_sum if abs_sum > 1e-12 else raw_w
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="AIPT P=1000 Paper Trader (KuCoin)")
